...
**`incomplete resolution – missing/null fields` (10 pts sub-check)**
- All 14 required fields must be present and non-null: `ticket_id`, `timestamp`, `customer_tier`, `subject`, `message`, `retrieved_kb_chunks`, `top_kb_source`, `category`, `priority`, `suggested_solution`, `kb_facts_used`, `retrieval_confidence`, `processing_time_ms`, `solution_contains_kb_keywords`.

**`expected value not in solution` (40 pts sub-check)**
...
- Ensure the Dataflow pipeline:
  ...
  3. Correctively maps the LLM output to the BigQuery schema.

**`retrieval_confidence too low` (< 0.4) or `retrieved_kb_chunks is empty`**
- The Dataflow pipeline is failing the RAG step. Verify it calls `retrieve_kb` (or equivalent BQ vector search) and retrieves at least one relevant chunk.
